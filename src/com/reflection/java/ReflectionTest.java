...
public class ReflectionTest {
    ...
    1. 类的加载过程:
    程序在经过javac.exe命令(编译)后，会生成一个或多个字节码文件 (.class)结尾。----编译
    接着我们使用java.exe命令对某个字节码文件进行解释运行。相当于讲某个字节码文件加载到内存中。----此过程就称为"类的加载"。
    加载到内存中的类，我们就称为运行时类 ---此运行时类，就作为Class的一个实例。
    2. 换句话说，Class的实例对应着一个运行时类。
    3. 加载到内存中的运行时类，会缓冲一定的时间。在此时间之内，我们可以通过不同的方式来获取此运行时类。
